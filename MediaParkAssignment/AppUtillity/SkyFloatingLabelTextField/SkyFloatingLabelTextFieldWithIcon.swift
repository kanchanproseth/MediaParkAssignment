...
open class SkyFloatingLabelTextFieldWithIcon: SkyFloatingLabelTextField {
    ...
    */
    override open func textRect(forBounds bounds: CGRect) -> CGRect {
        ...
        let isIconToRight = ((self.rightView?.bounds.width ?? 0.0) == 0.0) ? true : false
        if isLTRLanguage {
            rect.origin.x +=  CGFloat((isIconToRight ? iconWidth : 0) + iconMarginLeft)
        } else {
            rect.origin.x -= CGFloat((isIconToRight ? iconWidth : 0) + iconMarginLeft)
        }
        rect.size.width -= CGFloat((isIconToRight ? iconWidth : 0) + iconMarginLeft)
        return rect
    }
    
    /**
     Calculate the rectangleor the textfield when it is being edited
